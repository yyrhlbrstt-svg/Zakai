...
עמוד `/founder` מראה את משפך התיקים ובראשו **אחוז ההצלחה** (מתוך הפניות שנענו, כמה הניבו חיסכון מתועד). זה המספר שמאמת את כל המודל.
1. ב-Vercel: `ADMIN_EMAIL` = האימייל שלך (אפשר כמה, מופרד בפסיקים).
2. התחבר עם אותו אימייל וגלוש ל-`/he/founder`.
3. הרץ 20–30 תיקי סלולר אמיתיים וצפה שאחוז ההצלחה יציב לפני שמוסיפים ורטיקלים/שיווק.
