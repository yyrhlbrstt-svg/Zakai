# Fairness Layer — מיליארד בחודש (יישור מנכ"ל ↔ מוצר)
...
### 4. מיליארד בחודש — שלב 7

זה **פרוטוקול נפח עסקאות**, לא ARR של SaaS. דורש: מוסדות מאמתים Mandates, packs ב-CDN, widget מוטמע בבנקים, ו**אפס מספרים מומצאים** בפרסום. המסלול הכנה: B2C הוכחות → B2B2C widget → Fairness Score מבוסס outcome graph (כבר מתוכנן ב-`companyScore`).
