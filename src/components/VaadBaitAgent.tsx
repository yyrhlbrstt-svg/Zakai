...
export function VaadBaitAgent() {
  const t = useTranslations("vaadBait");
  const tFlow = useTranslations("agentFlow");
  const router = useRouter();
  const [name, setName] = useState("");
  const [buildingAddress, setBuildingAddress] = useState("");
  const [contactEmail, setContactEmail] = useState("");
  const [unexplainedCharge, setUnexplainedCharge] = useState("");
  const [chargeAmount, setChargeAmount] = useState("");
  const [busy, setBusy] = useState(false);
  const [caseId, setCaseId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const ready = Boolean(unexplainedCharge.trim() && hasOutreachEmail(contactEmail));

  async function sendWithAgent() {
    setError(null);
    setBusy(true);
    try {
      const res = await fetch("/api/cases/vaad-bait", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          unexplainedCharge,
          chargeAmountShekels: chargeAmount ? Number(chargeAmount) : undefined,
        }),
      });
      const data = await res.json().catch(() => ({}));
      if (res.status === 401) {
        router.replace(`/login?return=/vaad-bait`);
        return;
      }
      if (!res.ok) {
  ...
  return (
    <div className="mt-8">
      ...
      <Card className="p-6 flex flex-col gap-4">
        ...
        <MissingFields
          items={[
            { ok: Boolean(unexplainedCharge.trim()), label: t("agent.unexplainedCharge") },
