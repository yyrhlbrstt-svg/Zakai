...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "businessCompensation" });
...
export default async function BusinessCompensationPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("businessCompensation");
      ...
      <Reveal>
        <GradientCtaCard><div className="font-display text-xl">{t("cta.title")}</div>
            ...
            <div className="flex flex-wrap gap-3 justify-center mt-5">
              <Link href="/start?v=business-compensation">
