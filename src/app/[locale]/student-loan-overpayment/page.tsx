...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "studentLoanOverpayment" });
  return {
    title: t("metaTitle"),
    description: t("metaDesc"),
    alternates: { languages: alternateLanguages("/student-loan-overpayment") },
  };
}

/**
 * UK-first global revenue door: SLC overpayment refunds via the GB country pack
 * (letter template + cited source). No invented eligibility — user generates
 * the letter and sends it themselves; agent case loop can be added when GB
 * ops are live.
 */
export default async function StudentLoanOverpaymentPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("studentLoanOverpayment");
