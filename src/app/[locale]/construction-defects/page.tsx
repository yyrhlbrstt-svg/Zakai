...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "constructionDefects" });
...
export default async function ConstructionDefectsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("constructionDefects");
  const defects = t.raw("defects") as Item[];
  ...
  return (
    <main className="max-w-[820px] mx-auto px-5 pb-24 pt-5">
      ...
      <Reveal>
        <h2 className="font-display text-2xl mt-14 mb-4">{t("defectsTitle")}</h2>
      </Reveal>
