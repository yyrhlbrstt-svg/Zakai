...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "alimonyGuarantee" });
...
export default async function AlimonyGuaranteePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("alimonyGuarantee");
      ...
      <Reveal>
        <GradientCtaCard><div className="font-display text-xl">{t("cta.title")}</div>
            ...
            <div className="flex flex-wrap gap-3 justify-center mt-5">
              <Link href="/start?v=alimony-guarantee">
