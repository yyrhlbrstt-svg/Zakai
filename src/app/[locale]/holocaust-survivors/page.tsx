...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "holocaustSurvivors" });
...
/**
 * Deliberately NOT a /start?v=X LeadForm flow, unlike every other vertical
 * page. This population already has specialized, free navigators (the
 * Authority's own "Kol Zchut" hotline, Claims Conference, NGOs like Amuta
 * Aviv) who know the current criteria and amounts — Zakai does not, and
 * guessing wrong here is a worse failure mode than on any other vertical.
 * The honest, useful role is pointing to the right door, not opening a
 * Case Zakai isn't positioned to run. Charges nothing — see GROWTH.md's
 * "billion-dollar" review for why this explicitly stays fee-free.
 */
export default async function HolocaustSurvivorsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("holocaustSurvivors");
  const kinds = t.raw("kinds") as Item[];
  ...
  return (
    ...
    >
      ...
      <Reveal>
        <h2 className="font-display text-2xl mt-14 mb-4">{t("kindsTitle")}</h2>
      </Reveal>
        ...
        {kinds.map((k, i) => (
          <Reveal key={k.title} delay={i * 80}>
            <SpotlightCard className="p-6 h-full">
              ...
              <div className="font-extrabold text-[15.5px] mt-3">{k.title}</div>
              <div className="text-ink-soft text-[13.5px] mt-1.5 leading-relaxed">{k.body}</div>
            </SpotlightCard>
          </Reveal>
        ))}
      </div>
