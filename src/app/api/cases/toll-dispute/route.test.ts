...
function req(body: unknown) {
  return new Request("https://zakai.test/api/cases/toll-dispute", {
    method: "POST",
...
describe("POST /api/cases/toll-dispute", () => {
  beforeEach(() => {
  ...
  it("rejects an invalid reason", async () => {
    const res = await POST(req({ ...VALID_BODY, reason: "bogus" }));
    expect(res.status).toBe(400);
    expect(createExpressVerticalCase).not.toHaveBeenCalled();
  });

  it("always targets the Route 6 provider name", async () => {
    await POST(req(VALID_BODY));
    expect(createExpressVerticalCase.mock.calls[0][0].provider).toBe("כביש חוצה ישראל");
  });

  it("never invents an amount when none was given", async () => {
    await POST(req(VALID_BODY));
