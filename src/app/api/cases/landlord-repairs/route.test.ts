...
function req(body: unknown) {
  return new Request("https://zakai.test/api/cases/landlord-repairs", {
    method: "POST",
...
const VALID_BODY = {
  tenantName: "נועה",
  landlordName: "יעקב",
  landlordEmail: "yaakov@example.test",
  propertyAddress: "הרצל 10",
  defectDescription: "נזילה בתקרה",
};

describe("POST /api/cases/landlord-repairs", () => {
  beforeEach(() => {
  ...
  it("rejects a too-short defect description", async () => {
    const res = await POST(req({ ...VALID_BODY, defectDescription: "x" }));
    expect(res.status).toBe(400);
    expect(createExpressVerticalCase).not.toHaveBeenCalled();
  });

  it("never invents a repair-cost amount when none was given", async () => {
    await POST(req(VALID_BODY));
    expect(createExpressVerticalCase.mock.calls[0][0].amountShekels).toBe(0);
  });

  it("passes through a given repair-cost estimate", async () => {
    await POST(req({ ...VALID_BODY, estimatedRepairCostShekels: 500 }));
    expect(createExpressVerticalCase.mock.calls[0][0].amountShekels).toBe(500);
  });

  it("creates the case with the landlord as provider and returns it", async () => {
    const res = await POST(req(VALID_BODY));
  ...
  it("surfaces needsOutreachEmail when no landlord email is given", async () => {
    createExpressVerticalCase.mockResolvedValue({ ok: false, status: 400, error: "needsOutreachEmail" });
    const res = await POST(req({ ...VALID_BODY, landlordEmail: undefined }));
    const body = await res.json();
    expect(res.status).toBe(400);
