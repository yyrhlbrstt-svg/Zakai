...
function req(body: unknown) {
  return new Request("https://zakai.test/api/cases/water-bill", {
    method: "POST",
...
describe("POST /api/cases/water-bill", () => {
  beforeEach(() => {
  ...
  it("rejects a missing repair date", async () => {
    const res = await POST(req({ ...VALID_BODY, repairDate: "" }));
    expect(res.status).toBe(400);
    expect(createExpressVerticalCase).not.toHaveBeenCalled();
  });

  it("never invents a bill amount when none was given", async () => {
    await POST(req(VALID_BODY));
    expect(createExpressVerticalCase.mock.calls[0][0].amountShekels).toBe(0);
  });

  it("always targets the water corporation as provider", async () => {
    await POST(req(VALID_BODY));
    expect(createExpressVerticalCase.mock.calls[0][0].provider).toBe("תאגיד המים והביוב");
  });
  ...
  it("passes through whether repair proof is already available", async () => {
    await POST(req({ ...VALID_BODY, hasRepairProof: false }));
    expect(createExpressVerticalCase).toHaveBeenCalled();
  });
});
