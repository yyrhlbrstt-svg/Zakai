...
function req(body: unknown) {
  return new Request("https://zakai.test/api/cases/train-delay", {
    method: "POST",
...
describe("POST /api/cases/train-delay", () => {
  beforeEach(() => {
  ...
  it("rejects a missing travel date", async () => {
    const res = await POST(req({ ...VALID_BODY, travelDate: "" }));
    expect(res.status).toBe(400);
    expect(createExpressVerticalCase).not.toHaveBeenCalled();
  });

  it("never invents a claimed amount when none was given", async () => {
    await POST(req({ ...VALID_BODY, claimedAmountShekels: undefined }));
    expect(createExpressVerticalCase.mock.calls[0][0].amountShekels).toBe(0);
  });

  it("always targets Israel Railways as the provider", async () => {
    await POST(req(VALID_BODY));
    expect(createExpressVerticalCase.mock.calls[0][0].provider).toBe("Israel Railways");
  });
