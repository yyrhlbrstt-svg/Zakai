 ...
 * No external assets — works offline after download.
 */
...
<title>ייפוי כוח — זכאי ${escapeHtml(auth.code)}</title>
<style>
...
<body>
  <div class="sheet">
    ...
    <div class="row">
      ...
      <span class="value">${escapeHtml(auth.principalName)} · ${escapeHtml(auth.principalPhoneMasked)}</span>
    </div>
    <div class="row">
    ...
    <div class="row">
      ...
      <span class="value">${escapeHtml(providerName)}</span>
    </div>
    <div class="row">
      <span class="label">הופק בתאריך</span>
      <span class="value">${escapeHtml(issued)}</span>
    </div>

    <div class="section">
      <h2>היקף ההרשאה</h2>
      <p>${escapeHtml(auth.scope)}</p>
    </div>
    ...
    <div class="verify">
      ...
      <div style="margin-bottom:6px">
        ...
        <span class="code">${escapeHtml(auth.code)}</span>
      </div>
      <a href="${escapeHtml(verifyUrl)}">${escapeHtml(verifyUrl)}</a>
    </div>

    <div class="footer">
      מסמך זה הופק אוטומטית על ידי זכאי · ${escapeHtml(appUrl)}
    </div>
  </div>
  <script class="no-print">
    // Auto-open print dialog when opened as attachment in some browsers is unwanted;
