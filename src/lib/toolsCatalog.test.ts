...
describe("toolsCatalog", () => {
  it("keeps Tools dropdown tiny — Money lives in chrome CTA, not featured", () => {
    expect(FEATURED_TOOLS.length).toBeGreaterThanOrEqual(2);
    expect(FEATURED_TOOLS.length).toBeLessThanOrEqual(5);
    // /money is the product front door in Header chrome — never duplicate in Tools.
    expect(FEATURED_TOOLS.some((t) => t.href === "/money")).toBe(false);
    expect(TOOL_CATALOG.some((t) => t.href === "/money")).toBe(true);
    expect(FEATURED_TOOLS.every((t) => t.agentic === true)).toBe(true);
