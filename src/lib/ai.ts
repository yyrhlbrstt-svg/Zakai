    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: `This is a photo of an Israeli MOBILE phone bill. Extract the provider name, the monthly charge as a plain ILS number, and a short Hebrew plan description if visible. If the image is not a readable bill, set readable=false. Respond ONLY with JSON: {"provider":"...","amount":number_or_null,"plan":"...","readable":boolean}`,
          },
        ],
      },
    ],
...
async function aiRecommendation(input: RecommendationInput): Promise<Recommendation> {
  ...
  const msg = await anthropic.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `A customer in Israel pays ${input.amountShekels} ILS/month to ${input.providerLabel} for: "${input.plan || "a standard mobile plan"}".
You are Zakai, a consumer-advocacy AI agent (an automated tool, NOT a claimed human-level negotiation expert).
Produce:
1. One-sentence strategy in ${langName} (loyalty discount / downgrade / retention pricing). Framed as an approach, not a promise.
2. Realistic target monthly amount (plain number, lower than ${input.amountShekels}).
