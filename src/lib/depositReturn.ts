...
export function buildDepositDemandLetter(input: DepositLetterInput): string {
  ...
  return withFooter(
    [
      `לכבוד ${landlordName},`,
      "",
      agentLetterOpenHe(tenantName),
      "",
      `הלקוח/ה שכר/ה ממך את הדירה ברחוב ${propertyAddress}, פינה/תה אותה והחזיר/ה את המפתחות. עד היום לא הושב פיקדון השכירות בסך ${amount}.`,
      "",
      ...
      "",
      "ניתן לקזז מהפיקדון רק חוב מוכח בפועל — שכר דירה שלא שולם, חשבונות פתוחים על שם השוכר/ת, או נזק ממשי מעבר לבלאי סביר — ובכפוף להצגת אסמכתאות. החזקת הפיקדון ללא חוב מוכח וללא הנמקה בכתב אינה כדין.",
      "",
      "בקשה אחת: השבת מלוא הפיקדון בהקדם. ככל שיש קיזוז — פירוט בכתב ואסמכתאות לכל ניכוי בתוך זמן סביר. בהיעדר מענה, הלקוח/ה שומר/ת על הזכות לפנות לתביעות קטנות.",
      "",
      agentLetterCloseHe(tenantName),
    ].join("\n"),
  );
}
