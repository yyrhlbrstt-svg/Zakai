...
const JWKS_URL = process.env.ZAKAI_JWKS_URL || "https://zakai-3uxj.vercel.app/.well-known/zakai-jwks.json";
...
createServer(async (req, res) => {
  if (req.method === "GET" && req.url === "/health") {
    ...
    res.end(JSON.stringify({ ok: true, jwks: JWKS_URL }));
    return;
  }
  ...
  try {
    ...
    res.end(JSON.stringify({ accepted: true, mandate_jti: jti, intent: body.intent }));
  } catch (err) {
    res.writeHead(401, { "content-type": "application/json" });
    res.end(JSON.stringify({ error: "mandate_rejected", reason: String(err) }));
  }
}).listen(PORT, () => {
  console.log(`zakai inbound receiver on :${PORT} (JWKS ${JWKS_URL})`);
});
