...
5. Check revocation — prefer the signed status list when the token embeds `zkm.status` (`GET` + verify `statuslist+jwt`, bit at `idx`); otherwise live `GET /api/mandate/status/{jti}`. `revoked` → `401`; `unknown` / unreachable → `503 revocation_unknown` (never `accepted: true`). Remember `jti` for idempotency only after accept.
